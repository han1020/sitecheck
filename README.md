...
exclude_title:   # '제목에서만' 매칭 — 라벨 값에 정상 등장할 수 있는 단어용
  ...
  - 오픈뱅킹     # 저축은행중앙회 전체 점검의 사유('코어뱅킹/오픈뱅킹 스토리지 …')에 걸리던 사례
...
external_orgs:   # 제목에 자기 기관명 없이 이 이름이 있으면 '남의 점검' 안내로 제외
  - 금융결제원 / 코스콤 / 대법원 / 삼성패스 / ...
external_review: # 외부기관 작업이라도 버리지 않고 '검토 필요' 탭으로 보낼 기관
  - 금융결제원 / 코스콤          # 인증서 등 자사 서비스 영향 가능성 → 사람이 확인
...
- **`오픈뱅킹` exclude → exclude_title 이동**: 저축은행중앙회 전체 점검(사유 라벨 '코어뱅킹/오픈뱅킹 스토리지 할당 작업')이 라벨 매칭으로 통째로 빠지던 문제. 제목에 '오픈뱅킹'이 있는 공지만 제외하도록 축소.
- **`external_review` 신설**: 금융결제원·코스콤 작업 안내는 인증서 등 자사 서비스 영향 가능성이 있어, 외부기관 판정으로 버리지 않고 **'검토 필요' 탭**으로 보낸다 (이미 지난 점검은 스킵 탭). 검토 탭 제목 아래에 사유·일시를 함께 표시.
- **저축은행 12곳에 `저축은행중앙회` 별칭 추가** (sites.yaml): 저축은행 전산은 중앙회 통합금융정보시스템 위에서 돌므로 중앙회 작업 = 자기 점검. 푸른저축은행 등에서 중앙회 공지가 '외부 기관' 스킵되던 문제 해소. 저축은행 외 카테고리에서는 여전히 외부기관으로 판정.
- **`force_include_service` 신설**: 업무 라벨 값에 '인터넷뱅킹'이 있으면 어떤 제외 검사에도 걸리지 않고 무조건 점검 공지로 판단.

### 제목 클릭 → 본문 모달 + IBK투자증권 핸들러 복구 (2026-08-04)

- **감지 목록·검토 필요 탭에서 제목 클릭 시 공지 본문을 모달로 표시**: run JSON에 `body_text`(8000자 제한)를 저장. 스크린샷이 안 잡히는 사이트도 본문 확인 가능. 본문이 없는 항목(과거 JSON 포함)은 기존처럼 일반 텍스트.
- **IBK투자증권(KRST0225) 상세 수집 복구**: 사이트 변경으로 상세 POST 호출이 error.html 리다이렉트로 바뀌어 본문·스크린샷이 전부 비던 문제. 상세를 GET으로 호출하도록 수정.
