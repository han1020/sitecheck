...
- **감지 목록 탭**: 실행 회차별 감지 공지 조회 (제목·일시·업무·사유·원문·스크린샷). 파싱 의심값은 ⚠로 강조 — 업무가 기본값 폴백이거나 종료 시각을 못 잡은 경우 셀에 표시되므로 원문으로 교차 확인하세요. 🗑로 항목+스크린샷 삭제.
  - 이력 선택 옆 **🗑 이력 삭제**: 선택한 회차의 run JSON과 그 회차 스크린샷 파일을 삭제 (엑셀은 유지). 수집 중에는 거부.
- **검토 필요 탭**: 제목은 점검인데 본문이 비어(이미지 공지 등) 자동 판별이 불가한 항목. 캡처 확인 후 **공지추가**를 누르면 일시·업무·사유를 입력하는 팝업이 뜨고(수집기가 뽑아둔 값이 있으면 미리 채움), '엑셀에 추가'를 눌러야 엑셀 `점검` 시트 맨 위에 노란색 행으로 들어간다. 엑셀뷰가 같은 파일을 보고 있으면 즉시 갱신.
- **스킵 탭**: 점검 키워드에 걸렸으나 제외된 공지를 사유와 함께 표시 (외부기관 안내 / 제외 키워드 / 이미 지난 점검 / 일시 파싱 실패 등). 상단 **분류 탭**(은행·저축은행·증권·카드·공공, 건수 표기)으로 분류를 고르고, 그 아래 **기관 칩**(전체 + 해당 분류의 기관별 건수)으로 기관을 좁혀 잘못 제외된 공지가 없는지 검증. 특히 '일시 파싱 실패'는 놓친 점검일 수 있어 주기적 확인 권장.
...
- **업데이트 내역 탭**: README의 '최근 변경' 섹션을 월별로 보여준다. 최근 두 달은 펼쳐져 있고 그 이전 달은 월 제목을 눌러 펼친다. README를 수정하면 다음 새로고침에 반영.
...
| KRST0240 | 삼성증권 | KRST0279 | DB증권 (舊 DB금융투자) |
...
- **배경**: 검토 탭의 '공지추가'가 확인창만 띄우고 곧바로 엑셀에 행을 넣어, 일시·업무가 빈칸이고 사유는 제목 그대로라 엑셀뷰에서 다시 고쳐야 했다 (사용자 요청).
- **변경**: '공지추가' 클릭 시 **일시·업무·사유 입력 팝업**을 띄우고, 입력값을 `/api/excel/append`로 보내 엑셀 `점검` 시트 맨 위에 노란색 행으로 추가. 일시는 `schedule_text`, 업무는 `service_text`를 미리 채우고, 사유는 본문에서 뽑은 `reason_text`가 있으면 그것을, 시스템 설명('… 점검 여부 확인 필요', '강제 검토 마커', '외부 기관(…) 작업')이면 제목을 기본값으로 둔다. 사유가 비면 추가 거부. 추가 후 엑셀뷰가 같은 파일을 보고 있으면 자동 새로고침, 중복(같은 기관코드+사유)이면 안내만 하고 추가하지 않는 기존 규칙 유지. 서버 API 변경 없음.

### 대시보드 '업데이트 내역' 탭 추가 (2026-09-14)

- **배경**: 변경 이력이 README에만 있어 대시보드에서 바로 볼 수 없었다 (사용자 요청).
- **변경**: '제외 키워드' 탭 옆에 **업데이트 내역** 탭. `/api/changelog`가 README의 `## 최근 변경 (YYYY-MM)` 섹션들을 파싱해 월별로 묶고, `### 제목 (날짜)` 소제목이 있으면 소제목 단위로, 없으면(2026-06 이전) 최상위 불릿 하나를 항목 하나로 나눈다. 불릿 중첩·굵게·인라인 코드·링크·인용만 지원하는 간이 마크다운 렌더러(`_md_inline`/`_md_block`)를 서버에서 돌려 HTML로 보낸다. 최근 두 달은 펼침, 그 이전은 접힘(월 제목 클릭으로 토글). README를 고치면 탭도 따라 바뀐다.

### 스킵 탭 필터를 분류 탭 + 기관 칩으로 교체 (2026-09-14)

- **배경**: 사유·분류·기관 select 세 개를 조합해야 해서 특정 기관의 스킵 건을 훑기 번거로웠다 (사용자 요청).
- **변경**: 사유 select를 없애고, 분류(은행·저축은행·증권·카드·공공)를 서브탭으로 만들어 첫 탭이 자동 선택된다. 탭 아래에 그 분류에 속한 기관이 칩으로 나열되고(전체 + 기관별 건수) 칩을 누르면 해당 기관만 보인다. 스킵 사유는 각 행의 배지로 계속 표시된다. `webserver.py` `buildSkipFilter()`/`buildSkipSiteChips()`/`selectSkipCat()`/`selectSkipSite()`.

### 사명 변경·브랜드명 불일치로 자기기관 판별에서 탈락하던 증권사 3곳 보정 (2026-09-14)

- **배경**: 이베스트투자증권이 LS증권으로 이름을 바꿨는데 `sites.yaml`의 기관명은 `이베스트투자증권`, `list_url`은 죽은 옛 도메인(ebestsec.co.kr)으로 남아 있었다. 핸들러는 이미 ls-sec.co.kr을 수집하고 있었지만 공지 본문에는 'LS증권'만 나오므로, 자기기관 판별(2.2 단계 `is_own_institution_notice`)이 "본문에 이베스트투자증권 미등장"으로 실제 점검 공지를 외부 기관 안내로 버렸다 (예: 9/9 등록 '시스템 작업으로 인한 서비스 이용 일시 중단 안내(9/12)', 9/7 'KRX 애프터마켓 개설 및 NXT 제도 반영으로 인한 업무 일시 중단 안내').
- **증권 22곳 전수 점검**: 각 핸들러를 실행해 본문에 등장하는 회사명과 `sites.yaml` 기관명을 비교했다. 같은 유형이 2곳 더 있었다.
  - **DB금융투자(KRST0279)**: 2025 사명 변경 후 본문이 전부 'DB증권'. `name: DB증권`, `aliases: [DB금융투자, 디비금융투자, 디비증권]`, `list_url`을 핸들러가 실제 쓰는 dbsec.co.kr로. 옛 표기는 `institution_aliases` 명부에 추가.
  - **모바일증권 나무(KRST1247)**: NH투자증권의 모바일 브랜드라 본문이 'NH투자증권'으로 표기되어 본문 있는 공지 18건 전부 자기기관 미등장 판정. 8/9 '전산시스템 작업으로 인한 일부 서비스 중단 안내'가 실제로 탈락했다. 기관명은 그대로 두고 `aliases: [NH투자증권, 나무증권, NAMUH, 나무]` 추가, `list_url`을 mynamuh.com으로.
- **LS증권 수정**: `sites.yaml` KRST0265 `name: LS증권`, `aliases: [이베스트투자증권, 이베스트투자, 이베스트]`, `list_url: https://www.ls-sec.co.kr/`. 엑셀·대시보드 기관명도 LS증권으로 표시된다(이전 실행분 carryover 는 기관코드+일시로 맞추므로 영향 없음). `keywords.yaml` `institution_aliases`에 옛 표기를 넣어 다른 기관 공지에 '이베스트투자증권'이 나오면 외부기관으로 잡히게 했다.
...
### 엑셀 수동 수정분 유지 (2026-08-04)

- 엑셀의 **업무·사유를 직접 고쳐 저장하면 다음 수집에서도 그 값이 유지**된다. 같은 (기관코드, 점검 일시) 행이 이전 엑셀에 이미 있으면 재수집 값으로 덮어쓰지 않고 이전 값을 그대로 쓴다 (이전 값이 비어 있을 때만 새로 추출한 값으로 채움). 노란색 강조도 '진짜 신규'(이전 엑셀에 없던 건)에만 칠해지므로, 수정한 기존 행은 흰색으로 남는다.
- **일시(점검 시각)를 고치면 다른 행으로 인식**되어 새 행이 생긴다. 업무·사유만 고칠 것. 반대로 공지의 점검 일시가 실제로 바뀌면 새 건으로 잡혀 다시 노란색이 된다.
- 실행정보 시트의 '신규 감지 건수'도 노란색 행 수(진짜 신규)와 일치하도록 맞췄다.
